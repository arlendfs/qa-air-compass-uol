...
*** Test Cases ***
CT-48: Buscar Usuário Com ID Muito Longo
    ...
    Log    [CT-48 DEBUG] Status: ${res.status_code} | Body: ${res.text}    DEBUG
    Should Not Be Equal As Integers    ${res.status_code}    500

CT-49: Cadastrar Usuário Com Nome Muito Longo (500 chars)
    ...
    Log    [CT-49 DEBUG] Status: ${res.status_code} | Body: ${res.text}    DEBUG
    Should Not Be Equal As Integers    ${res.status_code}    500
...
CT-50: Cadastrar Usuário Com Unicode E Emoji No Nome
    ...
    Log    [CT-50 DEBUG] Status: ${res.status_code} | Body: ${res.text}    DEBUG
    Should Not Be Equal As Integers    ${res.status_code}    500
    IF    ${res.status_code} == 201
...
CT-51: Criar Produto Com Preço Zero
    ...
    Log    [CT-51 DEBUG] Status: ${res.status_code} | Body: ${res.text}    DEBUG
    Should Not Be Equal As Integers    ${res.status_code}    500
    IF    ${res.status_code} == 201
...
CT-52: Criar Produto Com Quantidade Zero
    ...
    Log    [CT-52 DEBUG] Status: ${res.status_code} | Body: ${res.text}    DEBUG
    Should Not Be Equal As Integers    ${res.status_code}    500
    IF    ${res.status_code} == 201
...
CT-53: Criar Produto Com Preço Máximo
    ...
    Log    [CT-53 DEBUG] Status: ${res.status_code} | Body: ${res.text}    DEBUG
    Should Not Be Equal As Integers    ${res.status_code}    500
    IF    ${res.status_code} == 201
...
CT-54: Carrinho Com Múltiplos Produtos (10 itens)
    ...
    Log    [CT-54 DEBUG] Status: ${res.status_code} | Body: ${res.text}    DEBUG
    Should Be Equal As Integers    ${res.status_code}    201
    Dictionary Should Contain Key    ${res.json()}    _id

CT-55: Requisições Concorrentes Para POST /usuarios
    ...
    Log    [CT-55 DEBUG] Status codes: ${status_list}    DEBUG
...
Inicializar Suite De Edge Cases
    [Documentation]    Cria sessão, obtém token de admin e prepara fixtures para a suite.
    Criar Sessão ServeRest
    ${token}    Pegar Token de Autenticação
    Set Suite Variable    ${TOKEN_EDGE}    ${token}
    # Gera string de 500 chars para CT-49 no setup — evita repetição no test case
...
Criar Dez Produtos
    [Documentation]    Cria 10 produtos distintos e retorna lista de IDs para CT-54.
    # Cada produto com quantidade=2 para suportar 1 unidade no carrinho sem esgotar estoque.
    [Arguments]    ${token}
    ${ids}    Create List
    FOR    ${i}    IN RANGE    10
...
Gerar Lista De Emails
    [Documentation]    Gera uma lista de N emails únicos para uso no CT-55.
    [Arguments]    ${n}
    ${emails}    Create List
    FOR    ${i}    IN RANGE    ${n}
        ${email}    Gerar Email Aleatório
...
Cancelar Carrinho Multi Se Existir
    [Documentation]    Cancela o carrinho multi-produto criado no CT-54.
